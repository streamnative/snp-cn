---
title: 通过 pulsarctl 使用 Pulsar Function
id: function-pulsarctl
category: user-guides
---

为 Pulsar 集群创建 Pulsar Function 后，可以根据需要更新或删除这个 function。本文介绍了如何使用 pulsarctl CLI （命令行工具）来更新和删除 Pulsar Function。有关 pulsarctl CLI（命令行工具）支持操作的完整列表，参见 [pulsarctl 命令参考](https://docs.streamnative.io/pulsarctl/v2.7.0.7/)。

# 创建 Pulsar Function

创建集群后，可以使用 `pulsarctl functions create` 命令将 Pulsar Function 部署到集群中。
如下示例显示了如何通过指定 JAR 包来为 `public` 租户和 `default` 命名空间来创建一个 function。
...
下表列出了用于创建 Pulsar Function 的所有可用字段。
...
schema-type | 内置 schema 类型或自定义 schema 类名称，用于 function 输出消息。 | <empty string>
...
topics-pattern | 定义主题列表的主题模式。命名空间下与主题模式匹配的主题将被消费。`--input` 和 `--topics-Pattern` 是互斥的。<br/>可在 `--customSerdeInputs` 中为模式添加 SerDe 类名（仅支持 Java 函数）。 |
...
要更新 Pulsar Function 的配置项，可以使用 `pulsarctl functions update` 命令来进行更新。
